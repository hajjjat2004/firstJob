...
app.post('/compare', async (req, res) => {
  let {sheet1, sheet2,name1,name2,firstempl,secempl,firstcompare,seccompare,row1,row2} = req.body
  ...
  for (let i = row2; i < sheetFile2.length; i++) {
    ...
    if (!fileOne[convert(sheetFile2[i][secempl + 1])] ){
      ...
      }else if(fileOne[convert(sheetFile2[i][secempl + 1])][firstcompare + 1] === sheetFile2[i][seccompare + 1]&&sheetFile2[i][seccompare + 1]!=180){
        let arr1 = [0, firstempl + 1, firstcompare + 1
